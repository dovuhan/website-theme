...
class Nav_Menu_Images_Admin extends Nav_Menu_Images {
	...
	public function enqueue_scripts() {
		...
		// Enqueue old script
		wp_enqueue_script( 'nmi-scripts', IORI_URI . '/inc/modules/iori-nav-menu/inc/nmi.js', array( 'media-upload', 'thickbox' ), '1', true );
		wp_localize_script(
			'nmi-scripts',
			'nmi_vars',
			array(
				'alert' => __( 'You need to set an image as a featured image to be able to use it as an menu item image', 'iori' ),
			)
		);
		add_thickbox();
		...
		if ( version_compare( $wp_version, '3.5', '>=' ) ) {
			wp_enqueue_media();
			wp_enqueue_script( 'nmi-media-view', IORI_URI . '/inc/modules/iori-nav-menu/inc/media-view.js', array( 'jquery', 'media-editor', 'media-views', 'post' ), '1.1', true );
		}
	...
	public function filter_walker( $walker ) {
		...
		require_once IORI_DIR . '/inc/modules/iori-nav-menu/inc/walker.php';
	...
	public function _wp_post_thumbnail_html( $content, $post_id ) {
		...
		if ( has_post_thumbnail( $post->ID ) ) {
			...
			$link = __( 'Upload menu item image', 'iori' );
		}
		...
		if ( isset( $_REQUEST['thumb_was'] ) && -1 == $_REQUEST['thumb_was'] ) {
			...
			$remove_link = ' | <a href="#" data-id="' . $post->ID . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $post->ID . ');return false;">' . esc_html__( 'Remove menu item image', 'iori' ) . '</a>';
	...
	function attachment_fields_to_edit( $form_fields, $post ) {
		...
		$form_fields['nmihover'] = array(
			...
			'value'        => $is_hover,
			'helps'        => __( 'Should this image be used on hover', 'iori' ),
			'show_in_edit' => false,
		);
		...
		$form_fields['nmiactive'] = array(
			'label'        => __( 'Used when active?', 'iori' ),
			'input'        => 'html',
			...
			'value'        => $is_active,
			'helps'        => __( 'Should this image be used when menu item is active', 'iori' ),
			'show_in_edit' => false,
		);
	...
	/**
	 * Iori custom menu field save
	 *
	 * @since 1.0
	 * @access public
	 *
	 * @uses update_post_meta() To save new item's custom field.
	 * @return object $post The post object data.
	 */
