...
class NMI_Walker_Nav_Menu_Edit extends Walker_Nav_Menu_Edit {
	...
	public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
		...
		?>
			...
			<p class="description description-wide nmi-design">
				<label for="edit-menu-item-design-<?php echo esc_attr( $item_id ); ?>">
					...
					<select id="edit-menu-item-design-<?php echo esc_attr( $item_id ); ?>" data-field="nmi-design" class="widefat" name="menu-item-design[<?php echo esc_attr( $item_id ); ?>]">
						<option value="default" <?php selected( $design, 'default', true ); ?>><?php esc_html_e( 'Default', 'iori' ); ?></option>
						<option value="full-width" <?php selected( $design, 'full-width', true ); ?>><?php esc_html_e( 'Full width', 'iori' ); ?></option>
						<option value="sized" <?php selected( $design, 'sized', true ); ?>><?php esc_html_e( 'Set sizes', 'iori' ); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-thin nmi-width" style="display:none;">
				<label for="edit-menu-item-width-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Dropdown Width', 'iori' ); ?><br>
			...
			<p class="description description-thin nmi-height" style="display:none;">
				<label for="edit-menu-item-height-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Dropdown Height', 'iori' ); ?><br>
			...
			<p class="description description-wide nmi-block" style="display:none;">
				<label for="edit-menu-item-block-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Megamenu Elementor Block for the dropdown', 'iori' ); ?><br>
					<select id="edit-menu-item-block-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-block[<?php echo esc_attr( $item_id ); ?>]">
						<option value="" <?php selected( $block, '', true ); ?>><?php esc_html_e( 'None', 'iori' ); ?></option>
					...
					<a href="<?php echo admin_url( 'post.php?post=' . $block . '&action=edit' ); ?>" 
										<?php 
										if ( empty( $block ) ) :
											?>
						style="display:none;"<?php endif ?> class="edit-block-link" target="_blank"><?php esc_html_e( 'Edit this block with Page Builder', 'iori' ); ?></a>
					<a href="<?php echo admin_url( 'post-new.php?post_type=dy_block' ); ?>" class="add-block-link" target="_blank"><?php esc_html_e( 'Add new', 'iori' ); ?></a>
				</label>
			</p>
		
			<p class="description description-wide nmi-icon">
				<label for="edit-menu-item-height-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Icon name', 'iori' ); ?><br>
			...
			<p class="description description-wide nmi-label-text">
				<label for="edit-menu-item-label-text-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Label text', 'iori' ); ?><br>
			...
			<p class="description description-wide nmi-label">
				<label for="edit-menu-item-label-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Label color', 'iori' ); ?><br>
					<select id="edit-menu-item-label-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-label[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="primary" <?php selected( $label, 'primary', true ); ?>><?php esc_html_e( 'Primary Color', 'iori' ); ?></option>
						<option value="secondary" <?php selected( $label, 'secondary', true ); ?>><?php esc_html_e( 'Secondary', 'iori' ); ?></option>
						<option value="red" <?php selected( $label, 'red', true ); ?>><?php esc_html_e( 'Red', 'iori' ); ?></option>
						<option value="green" <?php selected( $label, 'green', true ); ?>><?php esc_html_e( 'Green', 'iori' ); ?></option>
						<option value="blue" <?php selected( $label, 'blue', true ); ?>><?php esc_html_e( 'Blue', 'iori' ); ?></option>
						<option value="orange" <?php selected( $label, 'orange', true ); ?>><?php esc_html_e( 'Orange', 'iori' ); ?></option>
						<option value="grey" <?php selected( $label, 'grey', true ); ?>><?php esc_html_e( 'Grey', 'iori' ); ?></option>
						<option value="black" <?php selected( $label, 'black', true ); ?>><?php esc_html_e( 'Black', 'iori' ); ?></option>
						<option value="white" <?php selected( $label, 'white', true ); ?>><?php esc_html_e( 'White', 'iori' ); ?></option>
					</select>
				</label>
			</p>
			<p class="description description-wide nmi-color-scheme">
				<label for="edit-menu-item-colorscheme-<?php echo esc_attr( $item_id ); ?>">
					<?php esc_html_e( 'Dropdown text color scheme', 'iori' ); ?><br>
					<select id="edit-menu-item-colorscheme-<?php echo esc_attr( $item_id ); ?>" class="widefat" name="menu-item-colorscheme[<?php echo esc_attr( $item_id ); ?>]">
						<option value=""></option>
						<option value="light" <?php selected( $color_scheme, 'light', true ); ?>><?php esc_html_e( 'Light', 'iori' ); ?></option>
						<option value="dark" <?php selected( $color_scheme, 'dark', true ); ?>><?php esc_html_e( 'Dark', 'iori' ); ?></option>
					</select>
				</label>
			</p>
		...
		if ( has_post_thumbnail( $item_id ) ) {
			...
			$link_text      = __( 'Change menu item image', 'iori' );

			// For WP 3.5+, add 'remove' action link
			if ( version_compare( $wp_version, '3.5', '>=' ) ) {
				$ajax_nonce  = wp_create_nonce( 'set_post_thumbnail-' . $item_id );
				$remove_link = ' | <a href="#" data-id="' . $item_id . '" class="nmi_remove" onclick="NMIRemoveThumbnail(\'' . $ajax_nonce . '\',' . $item_id . ');return false;">' . esc_html__( 'Remove menu item image', 'iori' ) . '</a>';
			}
		} else {
			$output   .= '<div class="nmi-current-image nmi-div" style="display: none;"></div>';
			$link_text = __( 'Upload menu item image', 'iori' );
		}
