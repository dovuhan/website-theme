(function($) {
    ...
    // ajax
    if ($("body").hasClass("iori-ajax-shop-on") || typeof $.fn.pjax != "undefined") {
        var that = this;
        
        $(document).pjax(
            ".widget_product_categories a, .product_list_widget a, .product-toolbar .sort-by a, .block-tags a, .view-mode a,  body.post-type-archive-product:not(.woocommerce-account) .woocommerce-pagination a, body.tax-product_cat:not(.woocommerce-account) .woocommerce-pagination a, .nav-previous a,  .nav-next a, .pagination a, .iori-woocommerce-layered-nav a, .woocommerce-widget-layered-nav-list a",
            "#iori-ajax-wrapper", {
                timeout: 5000,
                scrollTo: false,
            }
        ...
        $(document).on("pjax:start", function (xhr, options) {
            $("#primary").removeClass("ajax-loaded");
            $("#primary").addClass("ajax-loading position-relative");
        });
        
        $(document).on("pjax:complete", function (xhr, textStatus, options) {
            that.shopPageInit();
            scrollToTop(false);
            var tooltp = $(".iori-tooltip");
            if (tooltp.length > 0) {
                tooltp.tooltip();
            }
            $("#primary").removeClass("ajax-loading");
        });
        
        $(document).on("pjax:end", function (xhr, textStatus, options) {
            $("#primary").removeClass("ajax-loading position-relative");
            $("#primary").addClass("ajax-loaded");
        });
    }
    /*-----------------
    Sidebar Sticky
...
//Perfect Scrollbar
if (jQuery(".mobile-header-wrapper-inner").length) {
